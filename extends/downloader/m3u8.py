...
class m3u8(interface):
    ...
    def init(self):
        ...
        self.tmp_path = "%s/%s" % (os.path.join(Config.getInstance().APP_PATH, self.manage.getConfig('tmp_path')),
                ...
                if not self.isPause:
                    ...
                    if len(self.urlsbox) > 0 and self.thread_total < self.manage.getConfig('max_thread'):
                        ...
                        t = threading.Thread(target=self.down,
                                             args=(url, '%s/%s.ts' % (self.tmp_path, index)))
                        t.setDaemon(True)
                        t.start()
                        self.thread_total += 1
                if self.isStop:
    ...
    def down(self, url, tmpname):
        try:
            ...
            if len(self.key):
                ...
                if len(content):
                    with open(os.path.join(tmpname), 'ab') as f:
                        f.write(content)
                        f.flush()
                else:
